...
-- Not: Bağlantı stringinde mutlaka database=account_db olmalı!
...
BEGIN
    CREATE TABLE dbo.outbox_events (
        ...
        aggregate_type NVARCHAR(255) NOT NULL,  -- Hangi entity (Account, Transaction vs.)
        aggregate_id NVARCHAR(255) NOT NULL,    -- Entity'nin ID'si
        type NVARCHAR(255) NOT NULL,            -- Event tipi (AccountCreated, BalanceUpdated vs.)
...
SELECT @cdc_enabled = is_cdc_enabled FROM sys.databases WHERE name = 'account_db';
