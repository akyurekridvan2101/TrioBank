...
-- Not: Bağlantı stringinde mutlaka database=ledger_db olmalı!
...
BEGIN
    CREATE TABLE dbo.outbox_events (
        ...
        aggregate_type NVARCHAR(255) NOT NULL,  -- Hangi entity (Transaction, Balance vs.)
        aggregate_id NVARCHAR(255) NOT NULL,    -- Entity'nin ID'si
        type NVARCHAR(255) NOT NULL,            -- Event tipi (TransactionPosted, BalanceUpdated vs.)
...
SELECT @cdc_enabled = is_cdc_enabled FROM sys.databases WHERE name = 'ledger_db';
